'''
Command 
tem intenção de encapsular uma solicitação como um objeto,
 desta forma permitindo parametrizar clientes com diferentes solicitações,
 enfileirar ou fazer regsitro (log) de solicitações e suportar
 operações que podem ser desfeitas.
 É formado por um cliente (que orquestra tudo), um invoker
 que invoca as solicitações), um ou város objetos de comando
 (que fazem a ligação entre o receiver e a ação a ser executada)
 e um receiver ( o objeto qu vai execuar a ação no final;)
'''
...
class Light:
    ...
    def on(self):
        print(f'{self.name}, em {self.room_name} está ligada')

    def off(self):
        print(f'{self.name}, em {self.room_name} está apagada')
        
    def change_color(self,color: str) -> None:
        self.color = color
        print(f'A cor da luz "{self.name}" agora é {self.color}')
...
if __name__ == "__main__":
    ...
    bathroom_light = Light('Luz do banheiro', 'Banherio')
    ...
    remote.global_undo()
    remote.global_undo()
    remote.global_undo() #default color
    remote.global_undo()
    remote.global_undo()
